...
from wechat_key import wechat_en, wechat_path, wechat_ua, wechat_imei, wechat_me, wechat_media
...
def wickrDB(path, password):
    ...
    # Wickr_ConvoUser
    convuserColname = {'vGroupID':'cvu 채팅방 번호', 'serverIDHash':'사용자 ID 해시'} #,'userImageSecure':'프로필 사진'
            ...
            if value != '':
                try:
                    ...
                    if not os.path.exists(value):
                        urllib.request.urlretrieve(imageUrl, value)
                except:
                    value = path + 'purpleMedia/' + iname
...
def KaKaoTalk_DB_2(path):
    ...
    friendsColname = {'myID':'내 ID', 'id':'사용자 ID','name':'이름','nick_name':'닉네임','phone_number':'핸드폰 번호',
                    'status_message':'상태메시지','original_profile_image_url':'프로필 이미지'}
...
def KaKaoTalk_DB_1(path, rowlist):
    ...
    # chat_rooms
    roomColname = {'id':'채팅방 id','private_meta':'채팅방 이름','members':'채팅방 인원','last_updated_at':'시간',
                'last_log_id':'마지막 보낸사람','type':'채팅 타입','last_message':'마지막 보낸 메시지'}
...
def wechat_db(path):
    ...
    if not os.path.exists(f'{path}WCMedia/image'):
        rpath='/'.join(dbfile.replace('\\','/').split('/')[:-1])
        wechat_media(path, rpath)

    userColname = {'username':'id','nickname':'이름'}
    userRowlist = export(app, cur, 'rcontact', userColname, None, path)

    roomColname = {'username':'사용자 이름', 'member':'채팅방 인원', 'msgCount':'메시지 개수', 'conversationTime':'마지막 보낸 시간','content':'마지막 메시지'}
    roomRowlist = export(app, cur, 'rconversation', roomColname, userRowlist, path)

    imagecolname = {'thumbImgPath':'썸네일','bigImgPath':'원본사진'}
    imagerowlist = export(app, cur, 'ImgInfo2', imagecolname, None, path)

    encolname = {'createTime': '보낸 시간','isSend': '수/발신','talker': '보낸사람', 'receiver':'받는사람', 
                    'type': '파일타입','content': '메세지', 'imgPath': '미디어','chatid':'채팅방 인원'}
    enrowlist = export(app, cur, 'message', encolname, userRowlist, path)
    for er in enrowlist:
        for img in imagerowlist:
            if er[6] == img[0]:
                er[6] = img[1]

    colname = [encolname.values(), roomColname.values(), userColname.values()]
    rowlist = [enrowlist, roomRowlist, userRowlist]

    return colname, rowlist

def wechatConversation(row, colname, col_defs, compare, path, me):
    d_row = []
    send,receiver='',''
    for en, kr in colname.items():
        if kr == '받는사람' or kr == '채팅방 인원': value = me
        else: value = row[col_defs[en]]
        if value == None: value = ''
        ...
        if kr == '사용자 이름':
            ...
            if value != userid:
                value = f'{userid}, {value}'

        elif kr == '파일타입':
            type = value
            if value == 1: value = '메세지'
            elif value == 3: value = '사진'
            elif value == 43: value = '영상'
            elif value == 34: value = '음성 메세지'
            elif value == 48: value = '위치'
            elif value == 10000: value = '시스템 공지'
            else: pass

        elif kr == '수/발신':
            send = value
            if value == 0: value = '수신'
            elif value == 1: value = '발신'
            else: pass
            ...
            if value != receiver:
                value = f'{receiver}, {value}'
        elif kr == '보낸 시간' or kr == '마지막 보낸 시간':
            unixTimestamp = value / 1000
            value =datetime.fromtimestamp(int(unixTimestamp)).strftime('%Y-%m-%d %H:%M:%S')   
            ...
            if type == 34: # 음성메시지
                value = path+'WCMedia/msg_'+value+'.amr'
            elif type == 43: # 영상
                value = path+'WCMedia/'+value+'.mp4'
        elif kr == '원본사진':
            value = path+'WCMedia/image/'+value
        d_row.append(value)
...
def export(app, cur, table, colname, compare=None, path=None):
    ...
    if app == 'lysn':
        rowlist = [lysnConversation(row,colname,col_defs,compare,path) for row in rows]
    elif app == 'TongTong':
        rowlist = [tongtongConversation(row,colname,col_defs,compare,path) for row in rows]
    elif app == 'Wickr':
        rowlist = [WickrConversation(row,colname,col_defs,compare,path) for row in rows]
    elif app == 'Purple':
        rowlist = [purpleConversation(row, colname, col_defs, compare, path) for row in rows]
    elif app == 'WeChat':
        me = wechat_me(path)
        rowlist = [wechatConversation(row, colname, col_defs, compare, path, me) for row in rows]
    elif app == 'KakaoTalk':
        if table == 'friends':
            # 자신의 userid 찾기
            for i in range(len(rows)):
                ...
                if dec != '':
                    myID = rows[i][2]
                    break
            rowlist = [kakaoConversation(row,colname,col_defs,myID,path) for row in rows]
        else:
            rowlist = [kakaoConversation(row,colname,col_defs,compare,path) for row in rows]
