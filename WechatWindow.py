...
class WechatScreen(QDialog):
        ...
        for i in range(len(rowlist)):
            for j in range(len(rowlist[i])):
                if j == media:
                    tp = rowlist[i][types]
                    if tp == '사진':  # 사진
                        thumbnail = os.path.splitext(rowlist[i][j])[0].replace("THUMBNAIL_DIRPATH://", "")
                        mpath = self.path + 'Media/' + thumbnail

                        if os.path.isfile(mpath) == False:  # 썸네일 없을 경우
                            if os.path.isfile(self.path + 'Media/' + rowlist[i][j]) == False:  # 원본파일 없을 경우
                        ...
                        thumbnail = os.path.splitext(rowlist[i][j])[0]
                        mpath = self.path + 'Media/' + thumbnail

                        if os.path.isfile(mpath) == False:
                            if os.path.isfile(self.path + 'Media/' + rowlist[i][j] + ".mp4") == False:
                                mpath = 'image/noimage.png'
                            else:
                        ...
                        vpath = os.path.splitext(rowlist[i][j])[0].replace("msg_","")

                        if os.path.isfile(vpath) == False:
                            if os.path.isfile(self.path + 'Media/' + rowlist[i][j] + ".amr") == False:
    ...
    def imageWindow(self):
        ...
        mediaPath = self.path + 'Media/' + file
        image(mediaPath)

    def videoWindow(self):
        file = self.sender().text()
        mediaPath = self.path + 'Media/' + file + ".mp4"
        video(mediaPath)
